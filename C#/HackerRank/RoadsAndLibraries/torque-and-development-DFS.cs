...
class Solution 
{
    private static void PushAllToStack(Stack<int> stack, IEnumerable<int> values) {
    ...
    static long roadsAndLibraries(long n, long c_lib, long c_road, int[][] cities)
    {
        ...
        // Create a node->children structure so we can do a DFS of the graphs
        ...
        foreach (var city in cityMap.Keys) {
            ...
            PushAllToStack(searchStack, cityMap[city]);
            
            while (searchStack.Any()) {
                var searchCity = searchStack.Pop();                
                if (!visited.Contains(searchCity)) {
                    visited.Add(searchCity);
                    PushAllToStack(searchStack, cityMap[searchCity]);
                    roads++;
                    libraries--;
                }
            }
